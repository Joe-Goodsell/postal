...
impl Command {
    pub fn execute(&self, tui: &mut TUIComponents, app: &mut App) -> anyhow::Result<()> {
        match self {
            ...
            Command::NextControlFocus => executions::execute_cycle_control_forward(tui)?,
            Command::PrevControlFocus => executions::execute_cycle_control_backward(tui)?,
            Command::DismissPopup => executions::execute_dismiss_popup(tui)?,
